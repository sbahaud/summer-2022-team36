...
public class NewTripServlet extends HttpServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    float totalBudget = Float.parseFloat(StringEscapeUtils.escapeHtml4(request.getParameter("text-input-totalBudget")));
  ...
  public String validateInput(HttpServletRequest request){
    ...
    if(!request.getParameter("text-input-totalBudget").matches("^[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)$")){
        return "error";
    }
    return "";
  }
